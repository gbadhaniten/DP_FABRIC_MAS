"""GraphQLAPIAgent -- AI/Advanced Agent. GraphQL API endpoints exposing Fabric data to applications."""
...
class GraphQLAPIAgent(BaseAgent):
    """GraphQL API endpoints exposing Fabric data to applications."""
    ITEM_TYPE = "GraphQLAPI"
    ITEM_CODE = "GQLAPI"
    FAB_NOUN = "graphql-api"
    AGENT_FOLDER_NAME = "graphql-api-agent"

    def create(self, params: Dict[str, Any]) -> AgentResult:
        self._autotrain("create")
        dn = params.get("display_name", "Untitled_GraphQLAPI")
        ...
        if r.success: r.message = f"GraphQLAPI '{dn}' created"; r.data = {"display_name": dn, "workspace_id": ws}
        return r
