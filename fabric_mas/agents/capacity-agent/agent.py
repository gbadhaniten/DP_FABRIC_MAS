"""CapacityAgent -- Governance/Admin Agent. Fabric capacity management: SKU sizing, scaling, pause/resume, throttling."""
...
class CapacityAgent(BaseAgent):
    """Fabric capacity management: SKU sizing, scaling, pause/resume, throttling."""
    ITEM_TYPE = "Capacity"
    ITEM_CODE = "CAP"
    FAB_NOUN = "capacity"
    AGENT_FOLDER_NAME = "capacity-agent"

    def create(self, params: Dict[str, Any]) -> AgentResult:
        self._autotrain("create")
        dn = params.get("display_name", "Untitled_Capacity")
        ...
        if r.success: r.message = f"Capacity '{dn}' created"; r.data = {"display_name": dn, "workspace_id": ws}
        return r
