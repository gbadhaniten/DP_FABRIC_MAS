"""
EnvironmentAgent — Data Engineering Agent
=============================================
Custom Spark runtimes with managed Python/R/Jar libraries and Spark configuration.

Item: Environment (ENV)
CLI:  fab environment <verb> [options]
"""
...
class EnvironmentAgent(BaseAgent):
    ...
    ITEM_CODE = "ENV"
    FAB_NOUN = "environment"
    AGENT_FOLDER_NAME = "environment-agent"

    # ── CREATE ────────────────────────────────────────────────────────
    def create(self, params: Dict[str, Any]) -> AgentResult:
        """Create a new Environment in the workspace."""
        self._autotrain("create")
        display_name = params.get("display_name", "Untitled_Environment")
        ...
        if result.success:
            result.message = f"Environment '{display_name}' created in workspace {ws}"
    ...
    def analyze(self, item_id: Optional[str] = None, **kwargs: Any) -> AgentResult:
        """List or inspect Environment items."""
    ...
    def deploy(self, item_id: str, target: str, **kwargs: Any) -> AgentResult:
        """Deploy Environment to a target workspace."""
