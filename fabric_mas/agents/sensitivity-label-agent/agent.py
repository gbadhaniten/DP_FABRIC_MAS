"""SensitivityLabelAgent -- Governance/Admin Agent. Microsoft Information Protection sensitivity labels for data classification."""
...
class SensitivityLabelAgent(BaseAgent):
    """Microsoft Information Protection sensitivity labels for data classification."""
    ITEM_TYPE = "SensitivityLabel"
    ITEM_CODE = "SL"
    FAB_NOUN = "sensitivity-label"
    AGENT_FOLDER_NAME = "sensitivity-label-agent"

    def create(self, params: Dict[str, Any]) -> AgentResult:
        self._autotrain("create")
        dn = params.get("display_name", "Untitled_SensitivityLabel")
        ...
        if r.success: r.message = f"SensitivityLabel '{dn}' created"; r.data = {"display_name": dn, "workspace_id": ws}
        return r
