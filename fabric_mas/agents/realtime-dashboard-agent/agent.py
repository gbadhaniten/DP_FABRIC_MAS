"""
RealTimeDashboardAgent — Real-Time Intelligence Agent
=========================================================
Live KQL-powered dashboards with auto-refresh tiles.

Item: RealTimeDashboard (RTD)
CLI:  fab realtime-dashboard <verb> [options]
"""
...
class RealTimeDashboardAgent(BaseAgent):
    """Live KQL-powered dashboards with auto-refresh tiles."""

    ITEM_TYPE = "RealTimeDashboard"
    ITEM_CODE = "RTD"
    FAB_NOUN = "realtime-dashboard"
    AGENT_FOLDER_NAME = "realtime-dashboard-agent"

    # ── CREATE ────────────────────────────────────────────────────────
    def create(self, params: Dict[str, Any]) -> AgentResult:
        """Create a new RealTimeDashboard in the workspace."""
        self._autotrain("create")
        display_name = params.get("display_name", "Untitled_RealTimeDashboard")
    ...
    def analyze(self, item_id: Optional[str] = None, **kwargs: Any) -> AgentResult:
        """List or inspect RealTimeDashboard items."""
    ...
    def deploy(self, item_id: str, target: str, **kwargs: Any) -> AgentResult:
        """Deploy RealTimeDashboard to a target workspace."""
