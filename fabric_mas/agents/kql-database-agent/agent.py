"""
KQLDatabaseAgent — Real-Time Intelligence Agent
===================================================
Kusto-based real-time analytics database for time-series and log data.

Item: KQLDatabase (KQL)
CLI:  fab kql-database <verb> [options]
"""
...
class KQLDatabaseAgent(BaseAgent):
    """Kusto-based real-time analytics database for time-series and log data."""

    ITEM_TYPE = "KQLDatabase"
    ITEM_CODE = "KQL"
    FAB_NOUN = "kql-database"
    AGENT_FOLDER_NAME = "kql-database-agent"

    # ── CREATE ────────────────────────────────────────────────────────
    def create(self, params: Dict[str, Any]) -> AgentResult:
        """Create a new KQLDatabase in the workspace."""
        self._autotrain("create")
        display_name = params.get("display_name", "Untitled_KQLDatabase")
    ...
    def analyze(self, item_id: Optional[str] = None, **kwargs: Any) -> AgentResult:
        """List or inspect KQLDatabase items."""
    ...
    def deploy(self, item_id: str, target: str, **kwargs: Any) -> AgentResult:
        """Deploy KQLDatabase to a target workspace."""
