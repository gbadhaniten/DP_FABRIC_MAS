"""DeploymentPipelineAgent -- Governance/Admin Agent. CI/CD deployment pipelines for promoting Fabric items across Dev/Test/Prod."""
...
class DeploymentPipelineAgent(BaseAgent):
    """CI/CD deployment pipelines for promoting Fabric items across Dev/Test/Prod."""
    ITEM_TYPE = "DeploymentPipeline"
    ITEM_CODE = "DPL"
    FAB_NOUN = "deployment-pipeline"
    AGENT_FOLDER_NAME = "deployment-pipeline-agent"

    def create(self, params: Dict[str, Any]) -> AgentResult:
        self._autotrain("create")
        dn = params.get("display_name", "Untitled_DeploymentPipeline")
        ...
        if r.success: r.message = f"DeploymentPipeline '{dn}' created"; r.data = {"display_name": dn, "workspace_id": ws}
        return r
