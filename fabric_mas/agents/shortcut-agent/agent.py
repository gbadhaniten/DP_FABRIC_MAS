"""
ShortcutAgent — Data Engineering Agent
==========================================
Virtual pointers to external/internal data sources (ADLS Gen2, S3, Dataverse, cross-Lakehouse).

Item: Shortcut (SHC)
CLI:  fab shortcut <verb> [options]
"""
...
class ShortcutAgent(BaseAgent):
    ...
    ITEM_CODE = "SHC"
    FAB_NOUN = "shortcut"
    AGENT_FOLDER_NAME = "shortcut-agent"

    # ── CREATE ────────────────────────────────────────────────────────
    def create(self, params: Dict[str, Any]) -> AgentResult:
        """Create a new Shortcut in the workspace."""
        self._autotrain("create")
        display_name = params.get("display_name", "Untitled_Shortcut")
        ...
        if result.success:
            result.message = f"Shortcut '{display_name}' created in workspace {ws}"
    ...
    def analyze(self, item_id: Optional[str] = None, **kwargs: Any) -> AgentResult:
        """List or inspect Shortcut items."""
    ...
    def deploy(self, item_id: str, target: str, **kwargs: Any) -> AgentResult:
        """Deploy Shortcut to a target workspace."""
