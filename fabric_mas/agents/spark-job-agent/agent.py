"""
SparkJobDefinitionAgent — Data Engineering Agent
====================================================
Batch Spark jobs with configurable entry points, arguments, and compute.

Item: SparkJobDefinition (SJD)
CLI:  fab spark-job-definition <verb> [options]
"""
...
class SparkJobDefinitionAgent(BaseAgent):
    """Batch Spark jobs with configurable entry points, arguments, and compute."""

    ITEM_TYPE = "SparkJobDefinition"
    ITEM_CODE = "SJD"
    FAB_NOUN = "spark-job-definition"
    AGENT_FOLDER_NAME = "spark-job-agent"

    # ── CREATE ────────────────────────────────────────────────────────
    def create(self, params: Dict[str, Any]) -> AgentResult:
        """Create a new SparkJobDefinition in the workspace."""
        self._autotrain("create")
        display_name = params.get("display_name", "Untitled_SparkJobDefinition")
    ...
    def analyze(self, item_id: Optional[str] = None, **kwargs: Any) -> AgentResult:
        """List or inspect SparkJobDefinition items."""
    ...
    def deploy(self, item_id: str, target: str, **kwargs: Any) -> AgentResult:
        """Deploy SparkJobDefinition to a target workspace."""
