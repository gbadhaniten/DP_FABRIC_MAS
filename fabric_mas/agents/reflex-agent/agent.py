"""
ReflexAgent — Real-Time Intelligence Agent
==============================================
Reactive automation triggered by data conditions and streaming events.

Item: Reflex (RFX)
CLI:  fab reflex <verb> [options]
"""
...
class ReflexAgent(BaseAgent):
    ...
    ITEM_CODE = "RFX"
    FAB_NOUN = "reflex"
    AGENT_FOLDER_NAME = "reflex-agent"

    # ── CREATE ────────────────────────────────────────────────────────
    def create(self, params: Dict[str, Any]) -> AgentResult:
        """Create a new Reflex in the workspace."""
        self._autotrain("create")
        display_name = params.get("display_name", "Untitled_Reflex")
        ...
        if result.success:
            result.message = f"Reflex '{display_name}' created in workspace {ws}"
    ...
    def deploy(self, item_id: str, target: str, **kwargs: Any) -> AgentResult:
        """Deploy Reflex to a target workspace."""
