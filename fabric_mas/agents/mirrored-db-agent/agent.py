"""
MirroredDatabaseAgent — Analytics/Warehousing Agent
=======================================================
Mirror external databases (Azure SQL, Cosmos DB, Snowflake) into Fabric OneLake.

Item: MirroredDatabase (MDB)
CLI:  fab mirrored-database <verb> [options]
"""
...
class MirroredDatabaseAgent(BaseAgent):
    """Mirror external databases (Azure SQL, Cosmos DB, Snowflake) into Fabric OneLake."""

    ITEM_TYPE = "MirroredDatabase"
    ITEM_CODE = "MDB"
    FAB_NOUN = "mirrored-database"
    AGENT_FOLDER_NAME = "mirrored-db-agent"

    # ── CREATE ────────────────────────────────────────────────────────
    def create(self, params: Dict[str, Any]) -> AgentResult:
        """Create a new MirroredDatabase in the workspace."""
        self._autotrain("create")
        display_name = params.get("display_name", "Untitled_MirroredDatabase")
    ...
    def analyze(self, item_id: Optional[str] = None, **kwargs: Any) -> AgentResult:
        """List or inspect MirroredDatabase items."""
    ...
    def deploy(self, item_id: str, target: str, **kwargs: Any) -> AgentResult:
        """Deploy MirroredDatabase to a target workspace."""
