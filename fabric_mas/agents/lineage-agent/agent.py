"""DataLineageAgent -- Governance/Admin Agent. Track data flow dependencies and impact analysis across Fabric items."""
...
class DataLineageAgent(BaseAgent):
    """Track data flow dependencies and impact analysis across Fabric items."""
    ITEM_TYPE = "DataLineage"
    ITEM_CODE = "LIN"
    FAB_NOUN = "lineage"
    AGENT_FOLDER_NAME = "lineage-agent"

    def create(self, params: Dict[str, Any]) -> AgentResult:
        self._autotrain("create")
        dn = params.get("display_name", "Untitled_DataLineage")
        ...
        if r.success: r.message = f"DataLineage '{dn}' created"; r.data = {"display_name": dn, "workspace_id": ws}
        return r
