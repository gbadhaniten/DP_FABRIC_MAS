"""
OrgAppAgent — Reporting/Power BI Agent
==========================================
Organisation-wide Power BI application.

Item: OrgApp (OAPP)
CLI:  fab org-app <verb> [options]
"""
...
class OrgAppAgent(BaseAgent):
    """Organisation-wide Power BI application."""

    ITEM_TYPE = "OrgApp"
    ITEM_CODE = "OAPP"
    FAB_NOUN = "org-app"
    AGENT_FOLDER_NAME = "org-app-agent"

    # ── CREATE ────────────────────────────────────────────────────────
    def create(self, params: Dict[str, Any]) -> AgentResult:
        """Create a new OrgApp in the workspace."""
        self._autotrain("create")
        display_name = params.get("display_name", "Untitled_OrgApp")
    ...
    def deploy(self, item_id: str, target: str, **kwargs: Any) -> AgentResult:
        """Deploy OrgApp to a target workspace."""
