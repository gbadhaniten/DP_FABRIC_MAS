"""DomainAgent -- Governance/Admin Agent. Fabric domain boundaries for organising and governing workspaces."""
...
class DomainAgent(BaseAgent):
    """Fabric domain boundaries for organising and governing workspaces."""
    ITEM_TYPE = "Domain"
    ITEM_CODE = "DOM"
    FAB_NOUN = "domain"
    AGENT_FOLDER_NAME = "domain-agent"

    def create(self, params: Dict[str, Any]) -> AgentResult:
        self._autotrain("create")
        dn = params.get("display_name", "Untitled_Domain")
        ...
        if r.success: r.message = f"Domain '{dn}' created"; r.data = {"display_name": dn, "workspace_id": ws}
        return r
