"""
RealTimeHubAgent — Real-Time Intelligence Agent
===================================================
Central catalog and discovery hub for streaming data sources in Fabric.

Item: RealTimeHub (RTH)
CLI:  fab realtime-hub <verb> [options]
"""
...
class RealTimeHubAgent(BaseAgent):
    """Central catalog and discovery hub for streaming data sources in Fabric."""

    ITEM_TYPE = "RealTimeHub"
    ITEM_CODE = "RTH"
    FAB_NOUN = "realtime-hub"
    AGENT_FOLDER_NAME = "realtime-hub-agent"

    # ── CREATE ────────────────────────────────────────────────────────
    def create(self, params: Dict[str, Any]) -> AgentResult:
        """Create a new RealTimeHub in the workspace."""
        self._autotrain("create")
        display_name = params.get("display_name", "Untitled_RealTimeHub")
    ...
    def analyze(self, item_id: Optional[str] = None, **kwargs: Any) -> AgentResult:
        """List or inspect RealTimeHub items."""
    ...
    def deploy(self, item_id: str, target: str, **kwargs: Any) -> AgentResult:
        """Deploy RealTimeHub to a target workspace."""
